...
impl Plugin for GenerateRoomPlugin {
...
impl ValidRoomTiles {
    ...
    pub fn collapse(&self, rng: &mut RandomSource) -> Option<CollapsedRoomTiles> {
        let possibilities = [
            (self.gray, CollapsedRoomTiles::Gray),
            (self.red, CollapsedRoomTiles::Red),
            (self.yellow, CollapsedRoomTiles::Yellow),
            (self.green, CollapsedRoomTiles::Green),
            (self.lblue, CollapsedRoomTiles::LBlue),
            (self.dblue, CollapsedRoomTiles::DBlue),
        ]
...
impl Default for ValidRoomTiles {
    fn default() -> Self {
        Self {
            gray: true,
            red: true,
...
impl CollapsedRoomTiles {
    pub fn to_texture(&self) -> TileTextureIndex {
        TileTextureIndex(match self {
            CollapsedRoomTiles::Gray => 0,
            CollapsedRoomTiles::Red => 1,
            CollapsedRoomTiles::Yellow => 2,
            CollapsedRoomTiles::Green => 3,
            CollapsedRoomTiles::LBlue => 4,
            CollapsedRoomTiles::DBlue => 5,
        })
    }
}
...
fn create_origin(
    ...
    for tile_storage in &tilestorage_q {
        ...
        commands.entity(tile).insert(CollapsedRoomTiles::Red);
    }
}

fn update_neighbors(
    changed_tile_q: Query<(&CollapsedRoomTiles, &TilePos), Changed<CollapsedRoomTiles>>,
    tilestorage_q: Single<&TileStorage, With<RoomTilemap>>,
    mut valid_tile_q: Query<&mut ValidRoomTiles>,
) {
    ...
    for (collapsed, tile_pos) in changed_tile_q {
        ...
        for loc in neighbors.iter() {
            if let Some(entity) = tile_storage.checked_get(&loc) {
                ...
                match collapsed {
                    CollapsedRoomTiles::Gray => {
                        valid_tile.gray = false;
                    }

                    CollapsedRoomTiles::Red => {
                        valid_tile.red = false;
                    }

                    CollapsedRoomTiles::Yellow => {
                        valid_tile.yellow = false;
                    }
                    CollapsedRoomTiles::Green => {
                        valid_tile.green = false;
                    }

                    CollapsedRoomTiles::LBlue => {
                        valid_tile.lblue = false;
                    }

                    CollapsedRoomTiles::DBlue => {
...
fn collapse_tile(
    ...
    valid_tile_q: Query<&ValidRoomTiles>,
    ...
    let Some(collapsed) = valid_tile_q
        ...
        commands.entity(selected_entity).remove::<ValidRoomTiles>();
        return;
    };
